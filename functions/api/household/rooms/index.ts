...
import { conflictError, handleApiRequest, methodNotAllowed, parseJsonBody, requireD1, success } from "../../http";
...
export async function onRequestGet({ request, env }: Context) {
  return handleApiRequest(request, async (requestId) => {
    ...
    const rows = await db.prepare("SELECT id, name, description, display_order AS displayOrder FROM rooms WHERE household_id = ? AND is_active = 1 ORDER BY display_order, created_at")
...
export async function onRequestPost({ request, env }: Context) {
  return handleApiRequest(request, async (requestId) => {
    ...
    try {
      await db.prepare("INSERT INTO rooms (id, household_id, name, description, display_order, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?)")
        .bind(id, identity.householdId, name, description, order?.value ?? 0, now, now).run();
    } catch (error) {
      if (String(error).includes("UNIQUE constraint")) throw conflictError("An active Room with that name already exists.");
      throw error;
    }
    return success({ room: { id, name, description, displayOrder: order?.value ?? 0 } }, requestId, { status: 201 });
  });
}
