...
import { AnimatePresence, MotionDialog, MotionList, MotionListItem, MotionPage } from "./motion";
...
export function Meals({ navigate, signOut }: { navigate: (view: AuthenticatedView) => void; signOut: () => void }) {
  ...
  return <div className="dashboard-shell"><Navigation active="meals" navigate={navigate} signOut={signOut} /><MotionPage motionKey="meals" className="motion-page">
...
function MealEditor({ slot, suggestions, occasionDate, onOccasionDateChange, occasionSuggestions, busy, onClose, onSave, onQuickAction, onLoadOccasionSuggestions }: {
  ...
  return <MotionDialog className="meal-editor-sheet" role="dialog" aria-modal="true" aria-labelledby="meal-editor-title"><div className="meal-editor-head"><div><p className="eyebrow">Review meal</p><h2 id="meal-editor-title">{dayLabel(slot.dayOfWeek)} {slot.mealType}</h2></div><button onClick={onClose}>Close</button></div><p>Choose a favourite, add your own meal, or make space for real life.</p>
